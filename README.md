...
The contact rates, $c_{ij}$, and the transmission rates, $\beta_j$ are
specified for each age group with the `params` value (a named list), in
the entries whose names start with `c_` and `transmission_`,
respectively.

The current number of infectious individuals of each age group, $I_j$,
is computed over the course of the simulation. The population size of
each age group, $N_j$, is calculated internally from the initial state
vector, specified by the `state` value.
...
- `change-transmission`:
  ...
  - Scalar multiples of the original transmission rates are specified
    ...
    transmission rate to 50% then 10% of the original value across both
    age groups.
...
- `"change-contacts"`:
  ...
  - In intervention involves using a new contact matrix starting on the
    ...
    community contacts by 75% from the default `setting.weight` values.
