...
transval_o <- interventions_simulator$get$initial("transmission_o")[1,1]
transfactors_o <- c(1,0.5,0.3) # multiple of transmission rate for old

transvec_o <- transval_o*transfactors_o

transval_y <- interventions_simulator$get$initial("transmission_y")[1,1]
transfactors_y <- c(1,0.5,0.3) # multiple of transmission rate for young

transvec_y <- transval_y*transfactors_y

interventions_simulator$add$matrices(transmission_o_changepoints = trans_times)
interventions_simulator$add$matrices(transmission_y_changepoints = trans_times)

interventions_simulator$add$matrices(transmission_y_values = transvec_y)
interventions_simulator$add$matrices(transmission_o_values = transvec_o)

interventions_simulator$add$matrices(transmission_y_pointer = 0)
interventions_simulator$add$matrices(transmission_o_pointer = 0)

interventions_simulator$insert$expressions(
